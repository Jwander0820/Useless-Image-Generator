...
class MoveText:
    ...
    def generate_y_flow_element_param(img_shape, gif_list, text_list):
        ...
        repeat_time = 0
        location_x = random.randrange(0, weight, 10)
        ...
        output_string = MoveText.rt_random_list(text_list, choose_num)  # 生成要寫入的字串，從清單中隨機挑出指定數量
    ...
    def x_flow_random_number(img_shape=(1280, 720), location_y=100, move_step=10, word_size=1, choose_num=7):
        ...
        output_string = MoveText.rt_random_list(text_list, choose_num)  # 隨機挑選多少資料

        gif_list = MoveText.generate_frame(gif_img_shape, frame=60, gif_sec=5)  # 生成多少張資料
        ...
        for i in range(len(gif_list)):
            ...
            move = int(word_size * 25) * (-choose_num) * (n + 1) + move_step * i - (weight * n)
    ...
    def y_flow_random_number(img_shape=(1280, 720)):
        ...
        gif_list = MoveText.generate_frame(gif_img_shape, frame=60, gif_sec=5)
